...
def measure(input_ids):
    ...
    energy = meter.result.pkg[0] if meter and meter.result and meter.result.pkg else None
...
def run():
    ...
    for i in range(20):
        ...
        print(f"[FP32] {i}: {lat:.2f} ms | {thr:.2f} tok/s | Power: {power}")
    ...
    json.dump(out, open("results/fp32.json", "w"), indent=4)
